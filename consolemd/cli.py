...
def change_loglevel(ctx, param, value):
    ...
    if param.name == 'debug' and value:
        ...
        filter(lambda h: h.get_name() == 'stderr',
...
@click.option('-d', '--debug', is_flag=True, callback=change_loglevel,
              expose_value=True, is_eager=True, help="show extra info")
@click.option('-q', '--quiet', is_flag=True, callback=change_loglevel,
              expose_value=True, is_eager=True, help="show less info")
@click.option('--color/--no-color', default=True, callback=enable_color,
              is_eager=True, help="enable/disable logging color")
